...
const LABELS = {
 work: {
  ...
  descriptionPlaceholder:
   'Minden sor egy bullet pont lesz:\nFő feladataid\nElért eredményeid\nHasznált technológiák',
  icon: Briefcase,
  badge: 'Munkahely',
 },
 education: {
...
function SortableEntry({
 ...
 return (
  <>
   ...
   >
    {/* Accordion fejléc */}
    <div className="flex items-center gap-2 p-3 bg-slate-200 hover:bg-gray-100 transition-colors">
      ...
      <div className="space-y-2">
       ...
       {entry.startDate && !entry.endDate && !entry.current && (
        <p className="flex items-center gap-1.5 text-xs text-amber-600">
         ...
         Add meg a befejezés dátumát, vagy pipáld be a „{labels.currentLabel}"
         opciót
        </p>
       )}
      </div>
   ...
   {data.length === 0 ? (
    ...
    <>
     <p className="text-xs text-slate-600 mb-2">
      Húzd a <GripVertical className="size-3 inline" /> ikont a sorrend
      megváltoztatásához
     </p>
     <DndContext
      sensors={sensors}
